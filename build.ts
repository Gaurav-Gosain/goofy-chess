...
// Copy static assets that the server normally serves
const outdirStr = typeof outdir === 'string' ? outdir : path.join(process.cwd(), "dist");

// Chess piece GLB models
...
const stockfishSrcDir = path.resolve("node_modules", "stockfish", "bin");
copyFileSync(path.join(stockfishSrcDir, "stockfish-18-lite-single.js"), path.join(stockfishOutDir, "stockfish.js"));
copyFileSync(path.join(stockfishSrcDir, "stockfish-18-lite-single.wasm"), path.join(stockfishOutDir, "stockfish.wasm"));
console.log("📦 Copied Stockfish WASM files");
